<?php
 ...
 <?php
  ...
  <!-- Volunteer Profile List Table Data Load -->
 <?php
   $user_name    = $_SESSION['user_name'];
   $db           = new Database();
   $sql          = "SELECT * FROM tb_volunteer_profile";
   $read_profile = $db->select($sql);
  ?>
...
<head>
    ...
    <title>VMS - Admin Panel | Manage Profile</title>
    ...
    <div class="page-wrapper">
        ...
        <nav class="page-sidebar" id="sidebar">
            <div id="sidebar-collapse">
                ...
                <ul class="side-menu metismenu">
                    ...
                    <li>
                        <a class="active" href="manage_profile.php"><i class="sidebar-item-icon fa fa-address-book-o"></i>
                            <span class="nav-label">Volunteer Profile List</span>
                        </a>
                    </li>
                    <li>
                        <a href="blood_doner_list.php"><i class="sidebar-item-icon fa fa-tint"></i>
        ...
        <div class="content-wrapper">
            ...
            <div class="page-heading">
                <h1 class="page-title">Manage Volunteer Profile</h1>
                <ol class="breadcrumb">
            ...
            <div class="page-content fade-in-up">
                <div class="ibox">
                    <div class="ibox-head">
                        <div class="ibox-title">Profile List</div>
                    </div>
                    <div class="ibox-body">
                        <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <th>Profile Added On</th>
                                    <th>Profile Verify Status</th>
                                    <th>Active Profile</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if($read_profile){ $i = 0; ?>
                                <?php while($result = $read_profile->fetch_assoc()){ $i = $i + 1; ?>
                                <tr>
                                    <td class="text-primary"><?php echo '[VH-00'.$result['profile_id'].']'; ?></td>
                                    <td style="width: 10%;"><img src="<?php echo '../../../Volunteer_Panel/html/dist/'.$result['profile_img'] ?>" width="100" height="80" alt="volunteer_img"></td>
                                    <td><?php echo $result['name']; ?></td>
                                    <td><?php echo $result['profession']; ?></td>
                                    <td><?php echo $result['email']; ?></td>
                                    <td><?php echo $result['contact_number']; ?></td>
                                    <td><textarea class="form-control" rows="3"><?php echo $result['address']; ?></textarea></td>
                                    <td><?php echo date("F j, Y", strtotime($result['profile_added_on'])) ?></td>
                                    <td>
                                        <?php
                                            $profile_verify_status = $result['profile_verify_status'];

                                            if($profile_verify_status == 1)
                                            {
                                                $msg = '<div class="m-auto badge badge-success" style="border-radius: 15px;">Active</div>';
                                                    echo $msg;
                                            }
                                            else
                                            {
                                    ...
                                    <td>
                                        <?php
                                            ...
                                            {
                                            ?>
                                                <button type="button" class="btn btn-success btn-sm" name="button" disabled style="border-radius: 15px;"><i class="fa fa-check"></i> Activated</button>
                                            <?php
                                            }
                                            else
                                            {
                                            ?>
                                                <a href="volunteer_profile_verify.php?profile_id=<?php echo $result['profile_id']; ?>" onclick="return confirm('Sure to active profile?')" class="btn btn-success btn-sm" style="border-radius: 15px;"> Active Profile</a>
                                            <?php
                                            }
                                        ?>
                                    </td>
                                    <td>
                                        <a href="delete_data.php?profile_id=<?php echo $result['profile_id']; ?>" title="delete" onclick="return confirm('Are you sure to delete?')" class="btn btn-danger btn-sm px-2 py-1" style="border-radius: 3px;"><i class="fa fa-trash-o"></i></a>
                                    </td>
                                </tr>
                                <?php } ?>
                                <?php }else{ ?>
